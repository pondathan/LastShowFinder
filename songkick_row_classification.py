...
SF_METRO_SLUG_RE = re.compile(r"/metro-areas/\d+-(?:us-)?san-francisco-bay-area\b", re.I)
...
# Borough + token sets
NYC_BORO = {"new york", "manhattan", "brooklyn", "bklyn", "queens", "bronx", "staten island"}
NYC_FREE = {"nyc"}  # soft signal; only trust with nearby " NY "
SF_CITIES = {"san francisco", "oakland", "berkeley", "san jose", "palo alto", "mountain view", "santa clara", "daly city"}
...
def nearest_row(node: Tag) -> Tag:
    ...
    for _ in range(6):  # Look up to 6 levels up
        ...
        if p.find("time") and (p.find("a") or p.find(class_=re.compile(r"\blocation\b", re.I))):
            return p
        p = p.parent
...
def extract_songkick_row_candidate(
    ...
    if logger and metro != "NYC":
        ...
        if any(token in txt.lower() for token in ["ny", "new york", "brooklyn", "manhattan", "queens", "bronx"]):
    ...
    snippet = extract_venue_specific_text(row, venue) or " ".join(row.get_text(" ", strip=True).split())
